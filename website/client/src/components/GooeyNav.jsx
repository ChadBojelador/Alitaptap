...
const GooeyNav = ({
  ...
  const [localActive, setLocalActive] = useState(activeIndex >= 0 ? activeIndex : 0);

  useEffect(() => {
    const idx = items.findIndex(item => item.href === location.pathname);
    if (idx >= 0) setLocalActive(idx);
  }, [location.pathname]);
  ...
  const handleClick = (e, index, href) => {
    ...
    setTimeout(() => navigate(href), animationTime * 0.5);
  };
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(() => {
      const li = navRef.current?.querySelectorAll('li')[localActive];
  ...
  return (
    <>
      <style>{`
        ...
        .gooey-effect.text.active { color: #ffffff; }
        .gooey-effect.filter {
      ...
      <div style={{ position: 'relative' }} ref={containerRef}>
        <nav style={{ display: 'flex', position: 'relative', transform: 'translate3d(0,0,0.01px)' }}>
          <ul
            ref={navRef}
            style={{ display: 'flex', gap: '8px', listStyle: 'none', padding: '0 4px', margin: 0, position: 'relative', zIndex: 3, color: 'white' }}
          >
            {items.map((item, index) => (
              <li
                key={index}
                className={`gooey-li${localActive === index ? ' active' : ''}`}
              ...
              >
                <a
                  ...
                  style={{ outline: 'none', padding: '0.6em 1em', display: 'inline-block', textDecoration: 'none', color: 'inherit', fontSize: '14px', fontWeight: 500 }}
                >
                  {item.label}
                </a>
              </li>
