...
export default function Dashboard({ user }) {
  ...
  const handleChatSubmit = async (overrideInput) => {
    const userMessage = (overrideInput ?? chatInput).trim();
    if (!userMessage || !plan) return;
    setChatInput('');
    ...
    if (userMessage.toLowerCase().includes('solar') || userMessage.toLowerCase().includes('power')) {
      ...
      updatedPlan.tech_stack = { ...updatedPlan.tech_stack, hardware: (updatedPlan.tech_stack.hardware ? updatedPlan.tech_stack.hardware + ', ' : '') + 'Lithium-ion Battery Pack' };
