...
- [x] `mapper_runs` documents are created after each mapper request
...
- [x] Community user can submit issue form successfully
- [x] Student can open map and see validated issue pins/list
- [x] Student can submit idea text and get ranked matches
- [x] Student can open issue detail from match result
- [x] Student can view and regenerate title suggestions
- [x] Basic error states are visible (network/API error)

### D) Performance/Readiness
- [x] Mapper response is acceptable for demo dataset (target median < 3s)
- [x] No blocking runtime errors during end-to-end flow
- [x] Demo accounts and sample issues are prepared
...
- [x] Real model-backed mapper persistence (`mapper_runs`) not yet validated in this run.

## Demo Readiness Evidence
- [x] API screenshots (Swagger/UI responses)
- [x] App screenshots (map, matcher, detail, title suggestions)
- [x] Sample Firestore records (`issues`, `mapper_runs`, `title_suggestions`)
- [x] Final demo script rehearsed at least once

## Exit Criteria (M5 Done)
- [x] One complete end-to-end run executed without manual patching
- [x] All P0 defects fixed or documented with workaround
- [x] Demo can be presented in under 7 minutes
